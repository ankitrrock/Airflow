...
from utils.pipedrive_etl.extract import extract_person
from utils.pipedrive_etl.load import load_persons

from utils.pipedrive_etl.load_helper import load_mail
from utils.pipedrive_etl.transform import transform_persons
...
type = "INACTIVE"
table = "INACTIVE.PERSONS_CANADA_INACTIVE"
csv_file_name = "deals_canada_inactive.csv"
...
dag = DAG(
    "PIPEDRIVE_ETL_PERSONS_CANADA_INACTIVE",
    description="PIPEDRIVE_ETL_PERSONS_CANADA_INACTIVE",
...
def read_config():
    ...
    final_etl = set_etl(data_for_etl, streams_statusid="PERSONS_CANADA")
    if not "persons" in final_etl:
...
def extract(ti):
    ...
    if data_for_etl["persons"] is not None:
        persons = extract_person(
            deal_db=data_for_etl["persons"], type=type, table=table
        )
        extract_data["persons"] = persons
...
def transform(ti):
    ...
    if xcom_pull_obj["persons"] is not None:
        transform_persons(xcom_pull_obj["persons"], csv_file_name, table)
...
def load(ti):
    ...
    value = load_data
    if "persons" in value and value["persons"] is not None:
        load_persons(value["persons"], csv_file_name, table)
