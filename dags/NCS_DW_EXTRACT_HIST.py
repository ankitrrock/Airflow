...
dag = DAG('NCS_DW_EXTRACT_HIST'
          ,description='NCS_DW_EXTRACT_HIST'
...
def extraction_start():
    abc = glob.glob(Variable.get("UCM_DECRYPT_PATH_HIST") + "*")
    if len(abc) > 0:
        time = str(datetime.now()).split(".")[0].replace("-", "").replace(" ", "").replace(":", "")
        print(time)
        zip_path = '/home/oodles/ncs_multistage/apps/dwh_data/extracts/erp_extracts/archieve_files/hist/' + time + '/'
        print(zip_path)
        shutil.make_archive(zip_path, 'zip', Variable.get("UCM_DECRYPT_PATH_HIST"))
        for x in abc:
            os.remove(x)

    BATCH_NAME = "NCS_DW_EXTRACT_HIST"
    ...
    cur.execute(
        "INSERT INTO NCS_MDADM.KCA_CTRL_BATCH(BATCH_ID,BATCH_NAME,BATCH_LOAD_TYPE,BATCH_STATUS,BATCH_START_DATE,SRC_EXTR_START_DATE,SRC_EXTR_END_DATE,PRUNE_DAYS,SOURCE_APP_ID) SELECT   NCS_MDADM.SEQ_JOB_ID.NEXTVAL BATCH_ID,'" + BATCH_NAME + "' BATCH_NAME ,case when (select VALUE from NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST where PARAMETER_NAME='LOAD_TYPE') ='FULL' THEN 'FULL' ELSE 'INCR' END BATCH_LOAD_TYPE,'RUNNING' BATCH_STATUS,current_timestamp::timestamp_ntz BATCH_START_DATE ,case when (select VALUE from NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST where PARAMETER_NAME='LOAD_TYPE') ='FULL' THEN (select to_timestamp_ntz(VALUE,'YYYY-MM-DD HH:MI:SS') FROM NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST WHERE PARAMETER_NAME='SRC_EXTRACT_START_DATE') ELSE  NVL((SELECT to_timestamp(to_date(SRC_EXTR_END_DATE )-(SELECT  to_number(VALUE) FROM NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST WHERE PARAMETER_NAME='PRUNE_DAYS')|| ' '||to_time(SRC_EXTR_END_DATE)) FROM (SELECT SRC_EXTR_END_DATE FROM (SELECT SRC_EXTR_END_DATE,ROW_NUMBER() OVER (ORDER BY SRC_EXTR_END_DATE DESC) AS RN FROM  NCS_MDADM.KCA_CTRL_BATCH WHERE BATCH_NAME='" + BATCH_NAME + "' AND  BATCH_STATUS = 'COMPLETED' AND SRC_EXTR_END_DATE IS NOT NULL ) WHERE RN = 1)),(select to_timestamp_ntz(VALUE,'YYYY-MM-DD HH:MI:SS') FROM NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST WHERE PARAMETER_NAME='SRC_EXTRACT_START_DATE')) END SRC_EXTR_START_DATE,to_timestamp(to_char(current_date+1) ||' ' ||to_char(current_time)),(SELECT  to_number(VALUE) FROM NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST  WHERE PARAMETER_NAME='PRUNE_DAYS') PRUNE_DAYS,(select APP_ID FROM NCS_MDADM.KCA_CFG_APP_PARAM WHERE APPLICATION_NAME ='ERP' ) SOURCE_APP_ID")

    KCA_CFG_BATCH_PARAM = pd.read_sql("select * from NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST", con)
    KCA_CFG_BATCH_PARAM.to_excel(
        "/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_CFG_BATCH_PARAM.xlsx", index=False)

    KCA_CTRL_BATCH = pd.read_sql(
        "select * from NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_HIST' ORDER BY BATCH_START_DATE DESC LIMIT 1",
        con)
    KCA_CTRL_BATCH.to_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_CTRL_BATCH.xlsx",
                            index=False)

    query1 = "select * from NCS_MDADM.KCA_CFG_BATCH_DEP_HIST"
    data = pd.read_sql(query1, con)
    data.to_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/data.xlsx", index=False)

    PE_files = pd.read_sql(
        "select JOB_NAME FROM NCS_MDADM.KCA_CFG_BATCH_DEP_HIST WHERE JOB_NAME LIKE ('%_PE') AND DEPTH=0", con)
    PE_files.to_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/PE_files.xlsx", index=False)

    query2 = "select * from NCS_MDADM.KCA_ERP_TABLE_ROW_CNT_HIST order by ROW_COUNT DESC"
    data1 = pd.read_sql(query2, con)
    data1.to_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_ERP_TABLE_ROW_CNT.xlsx",
                   index=False)

    
    full_or_inc = cur.execute("select VALUE from NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST where PARAMETER_NAME='LOAD_TYPE'").fetchall()[0][0]

    data1 = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/data.xlsx")
    ...
    if full_or_inc=='FULL':
        for JOB_NAME in data_list1:
            trunc= "TRUNCATE TABLE NCS_ODS.{};".format(JOB_NAME)
...
def extraction_complete():
    ...
    # Inserting the running record into batch table to log the batch details
    cur.execute(
        "update NCS_MDADM.KCA_CTRL_BATCH set BATCH_STATUS='COMPLETED',BATCH_END_DATE=current_timestamp::timestamp_ntz WHERE BATCH_NAME='NCS_DW_EXTRACT_HIST' AND BATCH_ID=(select MAX(BATCH_ID) from NCS_MDADM.KCA_CTRL_BATCH);")
...
full_or_inc = cur.execute("select VALUE from NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST where PARAMETER_NAME='LOAD_TYPE'").fetchall()[0][0]
df = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_CFG_BATCH_PARAM.xlsx")

# full_or_inc="".join(df[df["PARAMETER_NAME"]=="LOAD_TYPE"]["VALUE"].tolist())
print("load type", full_or_inc)

df1 = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_CTRL_BATCH.xlsx")
...
en_path = str(Variable.get('UCM_ENCRYPT_PATH_HIST'))
de_path = str(Variable.get('UCM_DECRYPT_PATH_HIST'))

# query1="select JOB_NAME from EDWUAT.NCS_MDADM.KCA_CFG_BATCH_DEP_TEST where BATCH_NAME='KCA_DWH_LOAD' and DEPTH=0"
# data=pd.read_sql(query1,con)

data = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/data.xlsx")
...
x="/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_ERP_TABLE_ROW_CNT.xlsx"
bulk_file=[]
...
PE_files = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/PE_files.xlsx")
...
trigger_dag_ncs_dw_load_hist =TriggerDagRunOperator(task_id="trigger_ncs_dw_load_hist",trigger_dag_id="NCS_DW_LOAD_HIST",dag=dag)
...
dummy[n] >> bip_success >> ucm_start >> ucm1 >> ucm_success >> extraction_complete >> trigger_dag_ncs_dw_load_hist >> batch_error
...
df2 = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_ERP_TABLE_ROW_CNT.xlsx")
