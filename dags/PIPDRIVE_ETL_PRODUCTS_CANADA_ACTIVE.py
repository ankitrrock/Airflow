...
from utils.pipedrive_etl.extract import extract_product

from utils.pipedrive_etl.load import load_products
from utils.pipedrive_etl.load_helper import load_mail
from utils.pipedrive_etl.transform import transform_products

from airflow.operators.email_operator import EmailOperator


type = "ACTIVE"
table = "ACTIVE.PRODUCTS_CANADA_ACTIVE"
csv_file_name = "products_canada_active.csv"
...
dag = DAG(
    "PIPDRIVE_ETL_PRODUCTS_CANADA_ACTIVE",
    description="PIPDRIVE_ETL_PRODUCTS_CANADA_ACTIVE",
...
def read_config():
    ...
    final_etl = set_etl(data_for_etl, streams_statusid="PRODUCTS_CANADA")
    if not "products" in final_etl:
...
def extract(ti):
    ...
    if data_for_etl["products"] is not None:
        products = extract_product(
            deal_db=data_for_etl["products"], type=type, table=table
        )
        extract_data["products"] = products
...
def transform(ti):
    ...
    if xcom_pull_obj["products"] is not None:

        transform_products(xcom_pull_obj["products"], csv_file_name, table)
...
def load(ti):
    ...
    value = load_data
    if "products" in value and value["products"] is not None:

        load_products(value["products"], csv_file_name, table)
