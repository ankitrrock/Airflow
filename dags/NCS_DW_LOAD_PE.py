...
dag = DAG('NCS_DW_LOAD_PE'
          , description='NCS_DW_LOAD_PE'
...
def sql_runner(*op_args):
    ...
    if full_or_inc == full:
        ...
        file_path = Variable.get("PE_DECRYPT_PATH_HIST")
    ...
    # If depth is > 0, below code will trigger the sql file
        ...
        if full_or_inc == "FULL":
            ...
            start_date = cur.execute(
                "select SRC_EXTR_START_DATE from NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_PE' ORDER BY BATCH_START_DATE DESC LIMIT 1").fetchall()[
                0][0]
...
#Above code needs to be rechecked#
...
data_rows = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/bip_ucm_home/ucm/pe_files/excel_files/KCA_ERP_TABLE_ROW_CNT.xlsx")
...
filepath = Variable.get('PE_DECRYPT_PATH_HIST')
...
def load_config():
    ...
    cur.execute(
        "INSERT INTO NCS_MDADM.KCA_CTRL_BATCH(BATCH_ID,BATCH_NAME,BATCH_LOAD_TYPE,BATCH_STATUS,BATCH_START_DATE,SRC_EXTR_START_DATE,SRC_EXTR_END_DATE,PRUNE_DAYS,SOURCE_APP_ID)SELECT  (select MAX(BATCH_ID) from NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_PE') BATCH_ID,'" + BATCH_NAME + "' BATCH_NAME ,case when (select VALUE from NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST where PARAMETER_NAME='LOAD_TYPE') ='FULL' THEN 'FULL' ELSE 'INCR' END BATCH_LOAD_TYPE,'RUNNING' BATCH_STATUS,current_timestamp::timestamp_ntz BATCH_START_DATE ,(select SRC_EXTR_START_DATE from NCS_MDADM.KCA_CTRL_BATCH WHERE BATCH_NAME = 'NCS_DW_EXTRACT_PE' and BATCH_ID  IN (SELECT MAX(BATCH_ID) FROM NCS_MDADM.KCA_CTRL_BATCH)) SRC_EXTR_START_DATE,(select SRC_EXTR_END_DATE from NCS_MDADM.KCA_CTRL_BATCH WHERE BATCH_NAME = 'NCS_DW_EXTRACT_PE' and  BATCH_ID  IN (SELECT MAX(BATCH_ID) FROM NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_PE' and BATCH_STATUS='COMPLETED')) SRC_EXTR_END_DATE,(SELECT  to_number(VALUE) FROM NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST  WHERE PARAMETER_NAME='PRUNE_DAYS') PRUNE_DAYS,(select APP_ID FROM NCS_MDADM.KCA_CFG_APP_PARAM WHERE APPLICATION_NAME ='ERP' ) SOURCE_APP_ID")
...
def load_success():
    ...
    cur.execute("update NCS_MDADM.KCA_CTRL_BATCH set BATCH_STATUS='COMPLETED',BATCH_END_DATE=current_timestamp::timestamp_ntz WHERE BATCH_START_DATE IN(SELECT MAX(BATCH_START_DATE) FROM NCS_MDADM.KCA_CTRL_BATCH) AND BATCH_NAME='NCS_DW_LOAD_PE' AND BATCH_ID=(select MAX(BATCH_ID) from NCS_MDADM.KCA_CTRL_BATCH);")
...
def delete_old_files():
    ...
    encrypt_path='/home/oodles/ncs_multistage/export/home/dwhadminuser/bip_ucm_home/ucm/pe_files/encryptedfiles/'
