...
dag = DAG('NCS_DW_LOAD_HIST'
          , description='NCS_DW_LOAD_HIST'
...
def sql_runner(*op_args):
    ...
    if full_or_inc == full:
        ...
        file_path = Variable.get("UCM_DECRYPT_PATH_HIST")
    ...
    # If depth is > 0, below code will trigger the sql file
        ...
        if full_or_inc == "FULL":
            ...
            start_date = cur.execute(
                "select SRC_EXTR_START_DATE from NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_HIST' ORDER BY BATCH_START_DATE DESC LIMIT 1").fetchall()[
                0][0]
        ...
        # etl_job_id=cur.execute("select MAX(JOB_ID) FROM KCA_ERP.NCS_MDADM.KCA_CTRL_JOB WHERE JOB_TARGET='WRKFC_HD_CNT_HIST_FACT_STG' AND BATCH_ID IN (SELECT MAX(BATCH_ID) FROM KCA_ERP.NCS_MDADM.KCA_CTRL_BATCH)").fetchall()[0][0]
        ...
        with open(sql_file, 'r', encoding='utf-8') as f:
            ...
            sql = filter(None, sql.split(';'))
            for qrys in sql:
                if 'CALL NP_DW.KCA_WRKFC_HD_CNT_HIST_FACT_STG' in qrys:
                    cur.execute(qrys % get_param)
                else:
                    cur.execute(qrys)
...
data_rows = pd.read_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_ERP_TABLE_ROW_CNT.xlsx")
...
filepath = Variable.get('UCM_DECRYPT_PATH_HIST')
...
def load_config():
    ...
    cur.execute(
        "INSERT INTO NCS_MDADM.KCA_CTRL_BATCH(BATCH_ID,BATCH_NAME,BATCH_LOAD_TYPE,BATCH_STATUS,BATCH_START_DATE,SRC_EXTR_START_DATE,SRC_EXTR_END_DATE,PRUNE_DAYS,SOURCE_APP_ID)SELECT  (select MAX(BATCH_ID) from NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_HIST') BATCH_ID,'" + BATCH_NAME + "' BATCH_NAME ,case when (select VALUE from NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST where PARAMETER_NAME='LOAD_TYPE') ='FULL' THEN 'FULL' ELSE 'INCR' END BATCH_LOAD_TYPE,'RUNNING' BATCH_STATUS,current_timestamp::timestamp_ntz BATCH_START_DATE ,(select SRC_EXTR_START_DATE from NCS_MDADM.KCA_CTRL_BATCH WHERE BATCH_NAME = 'NCS_DW_EXTRACT_HIST' and BATCH_ID  IN (SELECT MAX(BATCH_ID) FROM NCS_MDADM.KCA_CTRL_BATCH)) SRC_EXTR_START_DATE,(select SRC_EXTR_END_DATE from NCS_MDADM.KCA_CTRL_BATCH WHERE BATCH_NAME = 'NCS_DW_EXTRACT_HIST' and  BATCH_ID  IN (SELECT MAX(BATCH_ID) FROM NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_HIST' and BATCH_STATUS='COMPLETED')) SRC_EXTR_END_DATE,(SELECT  to_number(VALUE) FROM NCS_MDADM.KCA_CFG_BATCH_PARAM_HIST  WHERE PARAMETER_NAME='PRUNE_DAYS') PRUNE_DAYS,(select APP_ID FROM NCS_MDADM.KCA_CFG_APP_PARAM WHERE APPLICATION_NAME ='ERP' ) SOURCE_APP_ID")
...
def load_success():
    ...
    cur.execute("update NCS_MDADM.KCA_CTRL_BATCH set BATCH_STATUS='COMPLETED',BATCH_END_DATE=current_timestamp::timestamp_ntz WHERE BATCH_START_DATE IN(SELECT MAX(BATCH_START_DATE) FROM NCS_MDADM.KCA_CTRL_BATCH) AND BATCH_NAME='NCS_DW_LOAD_HIST' AND BATCH_ID=(select MAX(BATCH_ID) from NCS_MDADM.KCA_CTRL_BATCH);")
    #cur.execute("UPDATE NCS_MDADM.KCA_CFG_BATCH_PARAM SET VALUE='INCR' WHERE PARAMETER_NAME='LOAD_TYPE' AND  VALUE='FULL'")
    #cur.execute("UPDATE NCS_MDADM.KCA_CFG_BATCH_PARAM SET VALUE=(SELECT TO_VARCHAR(DATEADD(day,-3,to_timestamp(MAX(SRC_EXTR_END_DATE))),'YYYY-MM-DD HH24:MI:SS') SRC_EXTR_END_DATE FROM NCS_MDADM.KCA_CTRL_BATCH WHERE BATCH_STATUS='COMPLETED' AND BATCH_NAME='KCA_DWH_EXTRACT' ORDER BY SRC_EXTR_END_DATE DESC) WHERE PARAMETER_NAME='SRC_EXTRACT_START_DATE';")
    
    #cur.execute("MERGE INTO NCS_MDADM.KCA_CFG_BATCH_DEP TGT USING (SELECT DEPTH,BATCH_NAME,JOB_NAME,TBL_MAX_RECORDS FROM NCS_MDADM.KCA_CFG_BATCH_DEP_HIST) SRC ON TGT.JOB_NAME=SRC.JOB_NAME WHEN NOT MATCHED THEN INSERT (DEPTH,BATCH_NAME,JOB_NAME,TBL_MAX_RECORDS) VALUES (SRC.DEPTH,SRC.BATCH_NAME,SRC.JOB_NAME,SRC.TBL_MAX_RECORDS);")
    
    #cur.execute("TRUNCATE NCS_MDADM.KCA_CFG_BATCH_DEP_HIST")
...
def delete_old_files():
    ...
    encrypt_path='/home/oodles/ncs_multistage/export/home/dwhadminuser/bip_ucm_home/ucm/encryptedfiles/hist/'
    ...
    archieve_path='/home/oodles/ncs_multistage/apps/dwh_data/extracts/erp_extracts/archieve_files/hist/'
