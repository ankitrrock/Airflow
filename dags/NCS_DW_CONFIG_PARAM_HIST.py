...
dag = DAG('NCS_DW_CONFIG_PARAM_HIST'
...
def excel_files():
    ...
    KCA_CFG_BATCH_PARAM.to_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_CFG_BATCH_PARAM.xlsx", index=False)

    KCA_CTRL_BATCH = pd.read_sql("select * from NCS_MDADM.KCA_CTRL_BATCH where BATCH_NAME='NCS_DW_EXTRACT_HIST' ORDER BY BATCH_START_DATE DESC LIMIT 1",con)
    KCA_CTRL_BATCH.to_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_CTRL_BATCH.xlsx",index=False)
    ...
    data1.to_excel("/home/oodles/ncs_multistage/export/home/dwhadminuser/excel_files/extract_dag/HIST/KCA_ERP_TABLE_ROW_CNT.xlsx",index=False)
...
def rowcount():
    ...
    file = glob.glob(Variable.get('UCM_DECRYPT_PATH_HIST') + "*.csv")
    for fi in file:
        print("fi is", fi)
        ...
        if re.sub('_UCM\w+.csv$', "", actual_filename) == "KCA_ERP_TABLE_ROW_CNT":
            ...
            con = snowflake.connector.connect(
                ...
                schema='NCS_MDADM')
            cur = con.cursor()
            cur.execute("TRUNCATE TABLE NCS_MDADM.KCA_ERP_TABLE_ROW_CNT_HIST")
            ...
            data1.to_sql('KCA_ERP_TABLE_ROW_CNT_HIST', con=engine, index=False, if_exists="append")

            connection.close()
            engine.dispose()
            con.close()
            destination = "/home/oodles/ncs_multistage/apps/dwh_data/extracts/erp_extracts/archieve_files/hist/"
...
trigger__dag_ncs_dw_extract_hist=TriggerDagRunOperator(task_id="Trigger_Dag_NCS_DW_EXTRACT_HIST",trigger_dag_id="NCS_DW_EXTRACT_HIST",dag=dag)

excel >> bip_row >> ucm_row >> row_count >> load_stat >> trigger__dag_ncs_dw_extract_hist
#excel >> bip_row >> ucm_row >> row_count >> load_stat