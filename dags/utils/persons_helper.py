...
tgt_cols = pd.read_csv(DATA_DIR+"/persons_columns.csv")["columns"].values
src_cols = map(lambda l: f"${l}", range(1, len(tgt_cols)+1))

def copy_data_from_csv_file_to_persons_table(csv_file_path, cur):
    ...
    copy_data_to_table = f"""copy into {MYDATABASE}.{MYSCHEMA}.PERSONS
                            (
                            {", ".join(tgt_cols)}
                            ) from
                            (
